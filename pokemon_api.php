...
function nombre_poke($id)
{
    $file_content = file_get_contents($GLOBALS["prefix_path"] . "collector_pokemon_association.json");
    $json_data = json_decode($file_content, true);
    $count = 0;
    foreach ($json_data as $entry) {
        if (isset($entry['pokemon_id']) && $entry["pokemon_id"] == $id && $entry['collector_id'] == get_current_id()) {
            $count++;
        }
    }
...
function verify_collector($id, $collector_name)
{
    $file_content = file_get_contents($GLOBALS["prefix_path"] . "collector_data.json");
    $json_data = json_decode($file_content, true);
    $result = "";
    if (array_key_exists($id, $json_data)) {
        $collec = $json_data[$id];
        if (in_array($collector_name, $collec)) {
...
function get_association_by_id($id)
{
    $file_content = file_get_contents($GLOBALS["prefix_path"] . "collector_pokemon_association.json");
    $json_data = json_decode($file_content, true);
    foreach ($json_data as $asso) {
        if ($asso["association_id"] == $id) {
            return $asso;
        }
    }
}
